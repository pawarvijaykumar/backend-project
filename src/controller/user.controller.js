...
const generateAccessAndRefereshTokens = async(userId) =>{
    try {
        ...
        await user.save({ validateBeforeSave: false })//not do the validetion directly fo and save 

        return{accessToken,refreshToken}//those token retunrd
...
const loginUser = asyncHandler(async (req, res) =>{
   ...
   const {accessToken, refreshToken} = await generateAccessAndRefereshTokens(user._id)
    ...
    const options = {
        httpOnly: true,//other cannnot modify the cookies,it can modify  only for server
        secure: true
    }
...
const logoutUser = asyncHandler(async(req, res) => {//logout method
...
const refreshAccessToken = asyncHandler(async (req, res) => {
    const incomingRefreshToken = req.cookies.refreshToken || req.body.refreshToken//if not give the response then use the if comndition

    if (!incomingRefreshToken) {
        throw new ApiError(401, "unauthorized request")//meane token is wr ong
    }
